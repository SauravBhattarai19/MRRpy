...
def _read_on_grid(path, shape, transform):
    """
    Read band 1 of *path* on the routing DEM's grid (*shape*, *transform*).

    A raster already of that shape is read as is.  One on the same pixel
    lattice but a larger extent is windowed to the DEM: the pyflwdir engine
    crops clipped_dem.tif / clipped_flow_accumulation.tif to the watershed
    bbox while flow_direction.tif / watershed.tif stay full-extent (the
    interactive delineate step needs the full grid).
    ...
    with rasterio.open(path) as src:
        ...
        st = src.transform
        col_off = (transform.c - st.c) / st.a
        row_off = (transform.f - st.f) / st.e
        same_res = (np.isclose(st.a, transform.a) and np.isclose(st.e, transform.e)
                    and st.b == 0 and st.d == 0)
        on_lattice = (abs(col_off - round(col_off)) < 1e-6
                      and abs(row_off - round(row_off)) < 1e-6)
        c0, r0 = int(round(col_off)), int(round(row_off))
        inside = (c0 >= 0 and r0 >= 0 and c0 + shape[1] <= src.width
                  and r0 + shape[0] <= src.height)
        if not (same_res and on_lattice and inside):
            raise ValueError(
                f"{path} (shape {src.shape}) is not on the routing DEM's grid "
                f"(shape {tuple(shape)}) and cannot be windowed to it — all "
                "routing rasters must share the DEM's resolution and pixel "
                "alignment.")
        window = Window(c0, r0, shape[1], shape[0])
        return src.read(1, window=window), src.nodata
...
def load_rasters(cfg):
    ...
    fdir, _ = _read_on_grid(cfg.ROUTING_FLOW_DIR_PATH, dem.shape, transform)
    faccum, nodata_fa = _read_on_grid(cfg.ROUTING_FLOW_ACCUM_PATH, dem.shape, transform)
    faccum = faccum.astype(np.float64)
    ws_raw, _ = _read_on_grid(cfg.ROUTING_WATERSHED_MASK_PATH, dem.shape, transform)
