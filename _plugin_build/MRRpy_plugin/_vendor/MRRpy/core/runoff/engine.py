...
class ScsCnMode(RunoffMode):
    ...
    def _scs_formula(self, P_mm):
        ...
        # xp.where evaluates both branches. CN=100 gives S=Ia=0, so the dry
        # initial state would otherwise evaluate 0/0 and emit a warning.
        denominator = xp.maximum(excess + self._S_1d, 1e-12)
        return xp.where(excess > 0, (excess ** 2) / denominator, 0.0)
