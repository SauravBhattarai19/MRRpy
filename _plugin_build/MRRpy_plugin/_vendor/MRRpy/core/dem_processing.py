    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "DELINEATION_ENGINE='pysheds' instead. "
...
def perform_hydrological_analysis(dem_path, output_point_latlon, target_crs_epsg, output_dir,
    ...
    engine : {'pysheds', 'pyflwdir'}
        Delineation engine.  Defaults to 'pyflwdir' (priority-flood fill);
        'pysheds' is the original engine.  See MRRpy.config.Config.DELINEATION_ENGINE.
    """
...
def analyze_terrain(dem_path, target_crs_epsg, output_dir, engine='pyflwdir'):
    ...
    ----------
    engine : {'pysheds', 'pyflwdir'}
        Delineation engine.  Defaults to 'pyflwdir' (matching Config); pass
        'pysheds' for the original engine.  'pyflwdir' uses priority-flood depression filling, which correctly
...
def delineate_from_outlet(output_dir, output_point_latlon, target_crs_epsg, engine='pyflwdir'):
    """
