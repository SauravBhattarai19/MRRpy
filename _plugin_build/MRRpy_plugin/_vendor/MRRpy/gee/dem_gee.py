...
GEE datasets
------------
    Any entry in ``MRRpy.gee.dem_catalog.DEM_CATALOG`` (NASADEM, SRTM,
    MERIT, ALOS AW3D30, Copernicus GLO-30, USGS 3DEP 1m, GMTED2010 — see
    ``MRRpy.describe_available_dems()`` or ``MRRpy list-dems``), or a
...
def download_dem(bbox_wgs84, target_crs_epsg, output_path, scale_m=None,
    ...
    dataset : str
        A catalog key from ``MRRpy.gee.dem_catalog`` (e.g. 'nasadem',
    ...
    try:
        ...
        if not band or not scale_m:
            raise ValueError(
                ...
                "MRRpy.list_available_dems()); pass both band= and "
