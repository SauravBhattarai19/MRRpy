...
runner.py
=========
Background worker that runs the MRRpy pipeline in a QThread so the
...
------------
    ...
    PipelineWorker(QThread)
        Signals:
            ...
            cancelled()     – the run stopped early at the user's request
...
class PipelineWorker(QThread):
    """
    Runs the full MRRpy pipeline (or individual stages) in a background
    thread.
    ...
    cancelled = pyqtSignal()            # stopped at the user's request (no result)
    ...
    def cancel(self):
        ...
        self.log.emit("[INFO] Cancel requested — the run stops after the current stage.")
    ...
    def _run_pipeline(self):
        ...
        # Exactly one of finished / cancelled / error is emitted per run, so the
        # dialog always gets a signal to re-enable its controls.
        if self._cancelled:
            self.cancelled.emit()
        else:
...
class DemStepWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            engine = self._params.get("engine", "pyflwdir")
            ...
            sys.stdout = original_stdout
            PipelineWorker._release_gpu_memory()
