...
#   'physical'     → Variable Source Area: Pradhan & Ogden (2010) OPM
...
# ═════════════════════════════════════════════════════════════════════════════
# 5.  OPM / VSA PARAMETERS  (used when RUNOFF_SOURCE = 'physical')
# ═════════════════════════════════════════════════════════════════════════════

# RUNOFF_MECHANISMS: which of the three runoff-generation mechanisms are active
#   (used when RUNOFF_SOURCE='physical').  Include any subset — they are fully
#   orthogonal, so this is the ONE place to compose the runoff model:
#     'saturation_excess'        → saturation-excess / Dunne (the OPM Variable Source Area)
#     'infiltration_excess'     → infiltration-excess via Green-Ampt (needs the OPM_GA_* params)
...
#   listed.  Examples: ['saturation_excess','impervious'] → no Horton; ['impervious'] → urban
...
# VSA_SD_MAX_INITIAL: root zone depth D — vertical height of the soil column
#   at the catchment divide [m].  When VSA_SD_SOURCE='gee', overridden by SERVES.
VSA_SD_MAX_INITIAL = 0.1    # m  (physical height, not water volume)

# VSA_SD_MIN: minimum saturation deficit [m] — floor on SD_max(t) as the sandbox
#   wets up (Rf = SD_min/SD_max drives Eq 4's A_t).  Only meaningful when 'saturation_excess'
#   is in RUNOFF_MECHANISMS (the sandbox is skipped entirely otherwise).
VSA_SD_MIN         = 0.001  # m

# VSA_Q_MAX: observed baseflow / initial discharge at the outlet [m³/s].
#   Used in Eq 10 (Pradhan & Ogden 2010) to calibrate the initial A_t.
VSA_Q_MAX          = 100    # m³/s  — set to observed pre-storm discharge

# VSA_PHI: drainable porosity [-].  When VSA_SD_SOURCE='gee', overridden by SoilGrids.
VSA_PHI            = 0.35

# VSA_K_SAT: field-scale lateral saturated hydraulic conductivity [m/day].
VSA_K_SAT          = 44.0   # m/day  (calibrated)

# VSA_PER_POLYGON: each gauge zone gets its own OPM sandbox (z, SD_max, A_t).
VSA_PER_POLYGON    = True
...
# that decides it, regardless of whether 'infiltration_excess' is also selected there):
...
#                  sandbox (z → SD_max(t) → A_t), whether or not 'infiltration_excess' is in
#                  RUNOFF_MECHANISMS.  Separately, when 'infiltration_excess' IS also in
...
# GA_SUCTION_SOURCE:
#   'scalar'  → uniform GA_SUCTION_M below (typical loam ≈ 0.1–0.2 m).
...
#               texture data fall back to GA_SUCTION_M.
GA_SUCTION_SOURCE = 'texture'
GA_SUCTION_M      = 0.15   # uniform value / nodata fallback [m]
...
#   lateral transmissivity VSA_K_SAT (44 m/day) that drives the sandbox Darcy
...
#
# GA_KSAT_SOURCE:
#   'scalar' → uniform GA_KSAT_MMHR below.
#   'gee'    → per-cell HiHydroSoil v2.0 Ksat (vertical saturated conductivity),
#              downloaded aligned to the routing DEM and cached to
#              GA_KSAT_RASTER.  Cells with no data fall back to GA_KSAT_MMHR.
#   'raster' → pre-computed mm/hr GeoTIFF at GA_KSAT_RASTER.
GA_KSAT_SOURCE = 'gee'
GA_KSAT_MMHR   = 12.0    # uniform value / nodata fallback [mm/hr]
GA_KSAT_RASTER = None    # None → auto-path: {OUTPUT_DIR}/ksat_hihydro.tif
...
# ── Baseflow ──────────────────────────────────────────────────────────────────
# VSA_BASEFLOW: seed every cell with the steady pre-storm discharge implied by
#   VSA_Q_MAX so the outlet hydrograph starts at baseflow instead of zero, making
...
# VSA_SD_SOURCE:
#   'manual' → use VSA_SD_MAX_INITIAL and VSA_PHI above
#   'gee'    → SERVES deficit + LULC/LCZ root zone depth + SoilGrids porosity
VSA_SD_SOURCE  = 'gee'

# VSA_SD_REDUCER: how to turn the per-cell SERVES deficit into one SD_max per
...
VSA_DEFICIT_RASTER = None   # None → auto-path: {OUTPUT_DIR}/deficit_serves_{date}.tif
...
SOILGRIDS_DEPTH = 'b30'        # 'b0' 'b10' 'b30' 'b60' 'b100' 'b200'
