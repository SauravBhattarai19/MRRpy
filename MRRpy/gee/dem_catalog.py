...
even without `pip install MRRpy[gee]`, so users can browse options before
deciding whether to install the extra. Only the actual download
(`MRRpy.gee.dem_gee.download_dem`) needs GEE auth.
...
def describe_dems():
    """Human-readable table of the catalog (used by `MRRpy list-dems`)."""
    lines = [
        "Available DEM sources (Google Earth Engine — requires "
        "'pip install MRRpy[gee]' + authentication to download):",
        "",
    ]
...
def get_dem_info(name):
    ...
    if key not in DEM_CATALOG:
        raise KeyError(
            ...
            "See MRRpy.describe_available_dems() for details."
        )
