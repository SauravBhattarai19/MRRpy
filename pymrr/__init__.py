# -*- coding: utf-8 -*-
"""
pymrr (Multi-Mechanism Runoff and Routing) — a distributed, physics-based
...
cli    : the ``pymrr`` command-line interface (config-file driven runs).
...
No local DEM yet? ``print(pymrr.describe_available_dems())`` lists the
DEM datasets pymrr can auto-download from Google Earth Engine (no
``[gee]`` install needed just to browse); set ``Config(DEM_BOUNDS_WGS84=...,
DEM_SOURCE=...)`` with an empty ``DEM_PATH`` to have the ``process_dem``
stage fetch one automatically (requires ``pip install pymrr[gee]`` +
authentication).
"""
