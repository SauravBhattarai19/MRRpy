...
===========================
Unit tests for Config (bridge/config_bridge.py).

These tests run with plain pytest — no QGIS installation required.
They verify that Config correctly mirrors every attribute in config.py
...
from MRRpy_plugin.bridge.config_bridge import Config  # noqa: E402


# ─────────────────────────────────────────────────────────────────────────────
class TestConfigDefaults:
    """Config default values match config.py shipped defaults."""
...
# ─────────────────────────────────────────────────────────────────────────────
class TestConfigMirrorsConfigPy:
    """Config must mirror every public UPPERCASE attribute in config.py."""
    ...
    def test_no_config_attribute_missing(self):
        ...
        mirror = set(Config().to_dict().keys())
        missing = real_attrs - mirror
        assert not missing, f"Config is missing config.py attributes: {sorted(missing)}"
...
class TestConfigKwargs:
    ...
    def test_single_override(self):
        cfg = Config(MANNINGS_N=0.05)
        assert pytest.approx(cfg.MANNINGS_N) == 0.05

    def test_multiple_overrides(self):
        cfg = Config(BACKEND="gpu", GPU_PRECISION="float32")
        assert cfg.BACKEND == "gpu"
        assert cfg.GPU_PRECISION == "float32"

    def test_invalid_attribute_raises(self):
        with pytest.raises(AttributeError, match="has no attribute"):
            Config(INVALID_PARAM=42)


# ─────────────────────────────────────────────────────────────────────────────
class TestConfigUpdateOutputPaths:
...
class TestConfigValidation:
    ...
    def test_missing_dem_raises(self):
        cfg = Config(DEM_PATH="/nonexistent/dem.tif")
        with pytest.raises(ValueError, match="DEM_PATH"):
            cfg.validate()

    def test_zero_time_step_raises(self):
        cfg = Config(TIME_STEP_SECONDS=0, DEM_PATH=__file__)
    ...
    def test_invalid_backend_raises(self):
        ...
        with pytest.raises(ValueError, match="BACKEND"):
            Config(BACKEND="tpu", DEM_PATH=__file__)
