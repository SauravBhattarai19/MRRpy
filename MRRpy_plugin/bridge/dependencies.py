...
dependencies.py
===============
Detect and install the third-party Python packages the MRRpy model needs,
...
REQUIRED = [
    ...
    # Not imported by name anywhere in MRRpy — pysheds and pyflwdir both pull
    ...
    # Imported unconditionally by the core: `import MRRpy` pulls in
    # MRRpy.plotting (matplotlib), and core/dem_processing.py imports
    # geopandas / shapely / pyproj at module level.  Keep this list in step
    # with [project].dependencies in pyproject.toml.
    ("matplotlib", "matplotlib", "hydrograph plots (imported by the core package)"),
...
OPTIONAL = [
    ("yaml",       "pyyaml",          "load / save .yaml config files"),
    ("ee",         "earthengine-api", "IMERG / SERVES / LULC / LCZ / GCN250 (Earth Engine)"),
]
