...
class TabDem(QWidget):
    ...
    # Emitted when an input that defines the watershed changes (DEM, CRS,
    # engine, outlet, output dir) — a previously delineated watershed is stale.
    watershed_stale = pyqtSignal()
    ...
    def _build_ui(self):
        ...
        # (display label, underlying MRRpy.config.Config.DELINEATION_ENGINE value)
        ...
        # A new outlet makes the delineated watershed stale (terrain is still valid).
        self.lat_spin.valueChanged.connect(self.watershed_stale.emit)
        self.lon_spin.valueChanged.connect(self.watershed_stale.emit)

        self.pick_btn = QPushButton(QgsApplication.getThemeIcon("/mActionCapturePoint.svg"), "Pick on map")
        ...
        default_out = os.path.join(os.path.expanduser("~"), "MRRpy_plugin", "output")
        self.output_dir_widget.setFilePath(default_out)
        self.output_dir_widget.fileChanged.connect(self._invalidate_terrain)
    ...
    def _invalidate_terrain(self, *args):
        """DEM / CRS / engine / output dir changed → re-analyze the terrain."""
    ...
    def _toggle_map_pick(self, checked):
        ...
        if checked:
            ...
            self._iface.messageBar().pushMessage("MRRpy_plugin", "Click on the map to set the outlet point.", duration=5
            )
        else:
    ...
    def apply_config(self, cfg):
        """Populate widgets from an Config object."""
        if cfg.DEM_PATH:
